...
void print_conditions(std::ostream& out, size_t n, const std::string& condition = "\\#1") {
	size_t i = 0;
	while (i != n) {
		out << "\\diagramcount\\hspace{3cm}\\textbf{" << condition << "} ";
		++i;
		if (i != n) out << "& ";
	}
	out << " \\\\\n";
...
int main(int argc, char** argv) {
	...
	if (keys.find("--condition") != keys.end()) {
	...
	while (!fin.eof()) {
		...
		if (col == nr_cols - 1) {
			print_conditions(fout, nr_cols, condition);
...
void info() {
	...
	std::cout << "\tfen2tex [--i=<INPUTFILE>] [--o=OUTPUTFILE] [--cols=<NR_COLS>] [--rows=NR_ROWS] [--condtition=CONDITION]\n";
	...
	std::cout << "\t\tCONDITION -- specific condition for generated set of diagrams. Default value is '\\#1'";
}
